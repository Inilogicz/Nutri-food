  ...
  function App() {
    ...
    useEffect(() => {
      const fetchDietitians = async () => {
        try {
          console.log("Fetching dietitians from /api/proxy/dietitians");
          setLoading(true);
          ...
          if (!response.ok) {
            ...
            console.error("Failed to fetch dietitians:", response.status, errorText);
            throw new Error("Failed to fetch dietitians");
          }
  
          const data = await response.json();
          console.log("Dietitians fetched:", data.data.dietitian);
          ...
          console.error("Error in fetchDietitians:", err);
          setError(err instanceof Error ? err.message : "An error occurred");
        } finally {
          setLoading(false);
          console.log("Fetch dietitians completed, loading:", false);
        }
      };
    ...
    const handleStartConsultation = (dietitian: Dietitian) => {
      console.log("Dietitian selected:", {
        id: dietitian.id,
        name: dietitian.name,
        specialty: dietitian.specialty,
        online: dietitian.online,
      });
      ...
      <div>
        ...
        {showModal && selectedDietitian && (
          <ConsultationConfirmationModal
            ...
            onCancel={() => {
              console.log("Consultation modal cancelled");
              setShowModal(false);
            }}
          />
        )}
